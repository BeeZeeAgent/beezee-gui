  ...
  try {
    ...
    if (entry.type === 'item.completed' && entry.item?.type === 'agent_message') {
      return {
        ...
        message: {
          role: 'assistant',
          content: [{ type: 'text', text: entry.item.text || '' }],
        },
      };
    }
    ...
    if (entry.type === 'error') {
      return { type: 'result', result: entry.message || 'Codex error', subtype: 'error', is_error: true };
    }
...
registry.register({
  id: 'codex', name: 'Codex CLI', command: 'codex', protocol: 'direct', supportsStdin: false, closeStdin: true,
  supportedFeatures: ['streaming', 'system-prompt', 'resume'],
  buildArgs(prompt, config) {
    if (config?.resumeSessionId) {
      const args = ['-a', 'never', 'exec', 'resume', '--json', '--skip-git-repo-check', '--all'];
      if (config.model) args.push('--model', config.model);
      args.push(config.resumeSessionId);
      if (typeof prompt === 'string' && prompt.trim()) args.push(prompt);
      return args;
    }
    const args = ['-a', 'never', 'exec', '--json', '--skip-git-repo-check'];
    if (config?.model) args.push('--model', config.model);
    args.push('--sandbox', config?.sandbox || 'workspace-write');
    args.push(typeof prompt === 'string' ? prompt : String(prompt));
    return args;
  },
...
registry.register({ id: 'cli-opencode', name: 'OpenCode', command: 'opencode', protocol: 'acp', supportsStdin: false, npxPackage: 'opencode-ai', supportedFeatures: ['streaming', 'resume', 'acp-protocol'], buildArgs: () => ['acp'], protocolHandler: acpProtocolHandler });
...
registry.register({ id: 'cli-gemini', name: 'Gemini', command: 'gemini', protocol: 'acp', supportsStdin: false, npxPackage: '@google/gemini-cli', supportedFeatures: ['streaming', 'resume', 'acp-protocol'], buildArgs(prompt, config) { const args = ['--experimental-acp', '--yolo']; if (config?.model) args.push('--model', config.model); return args; }, protocolHandler: acpProtocolHandler });
...
registry.register({ id: 'cli-kilo', name: 'Kilo', command: 'kilo', protocol: 'acp', supportsStdin: false, npxPackage: '@kilocode/cli', supportedFeatures: ['streaming', 'resume', 'acp-protocol', 'models'], buildArgs: () => ['acp'], protocolHandler: acpProtocolHandler });
