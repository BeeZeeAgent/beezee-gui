...
import { Store } from '../node_modules/ccsniff/src/store.js';
...
function claudeProjectDirForCwd(cwd) {
  const normalized = path.resolve(String(cwd).replace(/^~(?=$|\/)/, os.homedir()));
  const projectsRoot = path.join(os.homedir(), '.claude', 'projects');
  const direct = path.join(projectsRoot, normalized.replace(/\//g, '-'));
  if (fs.existsSync(direct)) return direct;
  const dashed = path.join(projectsRoot, normalized.replace(/\//g, '-').replace(/_/g, '-'));
  if (fs.existsSync(dashed)) return dashed;
  return null;
}

function loadHistoryStoreForCwd(cwd) {
  const projectDir = claudeProjectDirForCwd(cwd);
  if (!projectDir) return null;
  const cached = historyStores.get(projectDir);
  if (cached) return cached;
  const store = new Store(projectDir);
  store.loadOnce();
  historyStores.set(projectDir, store);
  return store;
}

function titleFromSessionEvents(store, session) {
  const folder = path.basename(session.cwd || '');
  const current = (session.title || '').trim();
  if (current && current !== folder && current !== session.project && !current.startsWith('[agent]')) return current;
  const firstUser = store.sessionEvents(session.sid)
    .find(e => {
  ...
  return text ? text.slice(0, 96) : (current || session.sid);
}
...
export function register(router, deps) {
  ...
  router.handle('history.sessionsForCwd', (p) => {
    ...
    const projectDir = claudeProjectDirForCwd(cwd);
    const store = loadHistoryStoreForCwd(cwd);
    const sessions = store ? store.sessions()
      .filter(s => (s.cwd || '') === cwd)
      .filter(s => !projectDir || fs.existsSync(path.join(projectDir, s.sid + '.jsonl')))
      .map(s => ({ ...s, title: titleFromSessionEvents(store, s), agentId: 'claude-code' })) : [];
    return { cwd, sessions };
  });
  ...
  router.handle('history.sessionEventsForCwd', (p) => {
    ...
    const cwd = path.resolve(String(p.cwd).replace(/^~(?=$|\/)/, os.homedir()));
    const store = loadHistoryStoreForCwd(cwd);
    const events = store ? store.sessionEvents(p.sid) : [];
